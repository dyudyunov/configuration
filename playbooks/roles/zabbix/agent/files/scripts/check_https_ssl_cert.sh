...
# Get certificate
output=$( ( echo -e 'HEAD / HTTP/1.1\nHost: '$host'\nUser-Agent: Zabbix HTTPS check\n' 2> /dev/null ; sleep 2 ) \
...
if [ -z "$output" ]; then error "openssl s_client responce is empty" ; fi

# echo `date`" $0 $*:" >> /tmp/zabbix_ssl_checker.log
# echo "output: $output" >> /tmp/zabbix_ssl_checker.log

# Run checks
if [ "$check_type" = "expire" ]; then
	expire_date=$( echo "$output" 2> /dev/null | openssl x509 -noout -dates | grep '^notAfter' | cut -d'=' -f2 )
	...
	verify_return_code=$( echo "$output" 2> /dev/null | egrep '^[ ]+Verify return code:' | tr -s ' ' | cut -d' ' -f5 )
